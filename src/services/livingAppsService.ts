...
import type { Mitarbeiterverwaltung, Werkzeugverwaltung, AusleiheRueckgabe, WartungPruefung, CreateMitarbeiterverwaltung, CreateWerkzeugverwaltung, CreateAusleiheRueckgabe, CreateWartungPruefung } from '@/types/app';
...
export class LivingAppsService {
  ...
  static async getMitarbeiterverwaltungEntry(id: string): Promise<Mitarbeiterverwaltung | undefined> {
    ...
    const record = { record_id: data.id, ...data, createdat: data.created_at ?? '', updatedat: data.updated_at ?? null } as Mitarbeiterverwaltung;
    return enrichLookupFields([record], 'mitarbeiterverwaltung')[0];
  }
  static async createMitarbeiterverwaltungEntry(fields: CreateMitarbeiterverwaltung): Promise<MutationResult> {
    const data = await callApi('POST', `/apps/${APP_IDS.MITARBEITERVERWALTUNG}/records`, { fields: cleanFieldsForApi(fields as any, 'mitarbeiterverwaltung') });
    return { ...data, record_id: data.id };
  }
  static async updateMitarbeiterverwaltungEntry(id: string, fields: Partial<CreateMitarbeiterverwaltung>): Promise<MutationResult> {
    const data = await callApi('PATCH', `/apps/${APP_IDS.MITARBEITERVERWALTUNG}/records/${id}`, { fields: cleanFieldsForApi(fields as any, 'mitarbeiterverwaltung') });
    return { ...data, record_id: data.id };
  }
  ...
  // --- WERKZEUGVERWALTUNG ---
  static async getWerkzeugverwaltung(): Promise<Werkzeugverwaltung[]> {
    const data = await callApi('GET', `/apps/${APP_IDS.WERKZEUGVERWALTUNG}/records`);
    const records = Object.entries(data).map(([id, rec]: [string, any]) => ({
      record_id: id, ...rec,
      createdat: rec.created_at ?? '', updatedat: rec.updated_at ?? null,
    })) as Werkzeugverwaltung[];
    return enrichLookupFields(records, 'werkzeugverwaltung');
  }
  static async getWerkzeugverwaltungEntry(id: string): Promise<Werkzeugverwaltung | undefined> {
    const data = await callApi('GET', `/apps/${APP_IDS.WERKZEUGVERWALTUNG}/records/${id}`);
    const record = { record_id: data.id, ...data, createdat: data.created_at ?? '', updatedat: data.updated_at ?? null } as Werkzeugverwaltung;
    return enrichLookupFields([record], 'werkzeugverwaltung')[0];
  }
  static async createWerkzeugverwaltungEntry(fields: CreateWerkzeugverwaltung): Promise<MutationResult> {
    const data = await callApi('POST', `/apps/${APP_IDS.WERKZEUGVERWALTUNG}/records`, { fields: cleanFieldsForApi(fields as any, 'werkzeugverwaltung') });
    return { ...data, record_id: data.id };
  }
  static async updateWerkzeugverwaltungEntry(id: string, fields: Partial<CreateWerkzeugverwaltung>): Promise<MutationResult> {
    const data = await callApi('PATCH', `/apps/${APP_IDS.WERKZEUGVERWALTUNG}/records/${id}`, { fields: cleanFieldsForApi(fields as any, 'werkzeugverwaltung') });
    return { ...data, record_id: data.id };
  }
  static async deleteWerkzeugverwaltungEntry(id: string) {
    return callApi('DELETE', `/apps/${APP_IDS.WERKZEUGVERWALTUNG}/records/${id}`);
  }

  // --- AUSLEIHE_RUECKGABE ---
  static async getAusleiheRueckgabe(): Promise<AusleiheRueckgabe[]> {
    const data = await callApi('GET', `/apps/${APP_IDS.AUSLEIHE_RUECKGABE}/records`);
    const records = Object.entries(data).map(([id, rec]: [string, any]) => ({
      record_id: id, ...rec,
      createdat: rec.created_at ?? '', updatedat: rec.updated_at ?? null,
    })) as AusleiheRueckgabe[];
    return enrichLookupFields(records, 'ausleihe_rueckgabe');
  }
  static async getAusleiheRueckgabeEntry(id: string): Promise<AusleiheRueckgabe | undefined> {
    const data = await callApi('GET', `/apps/${APP_IDS.AUSLEIHE_RUECKGABE}/records/${id}`);
    const record = { record_id: data.id, ...data, createdat: data.created_at ?? '', updatedat: data.updated_at ?? null } as AusleiheRueckgabe;
    return enrichLookupFields([record], 'ausleihe_rueckgabe')[0];
  }
  static async createAusleiheRueckgabeEntry(fields: CreateAusleiheRueckgabe): Promise<MutationResult> {
    const data = await callApi('POST', `/apps/${APP_IDS.AUSLEIHE_RUECKGABE}/records`, { fields: cleanFieldsForApi(fields as any, 'ausleihe_rueckgabe') });
    return { ...data, record_id: data.id };
  }
  static async updateAusleiheRueckgabeEntry(id: string, fields: Partial<CreateAusleiheRueckgabe>): Promise<MutationResult> {
    const data = await callApi('PATCH', `/apps/${APP_IDS.AUSLEIHE_RUECKGABE}/records/${id}`, { fields: cleanFieldsForApi(fields as any, 'ausleihe_rueckgabe') });
    return { ...data, record_id: data.id };
  }
  static async deleteAusleiheRueckgabeEntry(id: string) {
    return callApi('DELETE', `/apps/${APP_IDS.AUSLEIHE_RUECKGABE}/records/${id}`);
  }

  // --- WARTUNG_PRUEFUNG ---
  static async getWartungPruefung(): Promise<WartungPruefung[]> {
    const data = await callApi('GET', `/apps/${APP_IDS.WARTUNG_PRUEFUNG}/records`);
    const records = Object.entries(data).map(([id, rec]: [string, any]) => ({
      record_id: id, ...rec,
      createdat: rec.created_at ?? '', updatedat: rec.updated_at ?? null,
    })) as WartungPruefung[];
    return enrichLookupFields(records, 'wartung_pruefung');
  }
  static async getWartungPruefungEntry(id: string): Promise<WartungPruefung | undefined> {
    const data = await callApi('GET', `/apps/${APP_IDS.WARTUNG_PRUEFUNG}/records/${id}`);
    const record = { record_id: data.id, ...data, createdat: data.created_at ?? '', updatedat: data.updated_at ?? null } as WartungPruefung;
    return enrichLookupFields([record], 'wartung_pruefung')[0];
  }
  static async createWartungPruefungEntry(fields: CreateWartungPruefung): Promise<MutationResult> {
    const data = await callApi('POST', `/apps/${APP_IDS.WARTUNG_PRUEFUNG}/records`, { fields: cleanFieldsForApi(fields as any, 'wartung_pruefung') });
    return { ...data, record_id: data.id };
  }
  static async updateWartungPruefungEntry(id: string, fields: Partial<CreateWartungPruefung>): Promise<MutationResult> {
    const data = await callApi('PATCH', `/apps/${APP_IDS.WARTUNG_PRUEFUNG}/records/${id}`, { fields: cleanFieldsForApi(fields as any, 'wartung_pruefung') });
    return { ...data, record_id: data.id };
  }
  static async deleteWartungPruefungEntry(id: string) {
    return callApi('DELETE', `/apps/${APP_IDS.WARTUNG_PRUEFUNG}/records/${id}`);
  }
