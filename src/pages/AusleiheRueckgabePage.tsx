...
export default function AusleiheRueckgabePage() {
  const navigate = useNavigate();
  const [records, setRecords] = useState<AusleiheRueckgabe[]>([]);
  const [loading, setLoading] = useState(true);
  const [search, setSearch] = useState('');
  const [dialogOpen, setDialogOpen] = useState(false);
  const [editingRecord, setEditingRecord] = useState<AusleiheRueckgabe | null>(null);
  const [deleteTarget, setDeleteTarget] = useState<AusleiheRueckgabe | null>(null);
  ...
  async function handleCreate(fields: AusleiheRueckgabe['fields']) {
  ...
  async function handleUpdate(fields: AusleiheRueckgabe['fields']) {
    ...
    await LivingAppsService.updateAusleiheRueckgabeEntry(editingRecord.record_id, fields);
    await loadData();
  ...
  async function handleDelete() {
    ...
    await LivingAppsService.deleteAusleiheRueckgabeEntry(deleteTarget.record_id);
  ...
  return (
    <PageShell
      ...
      subtitle={`${records.length} Ausleihe & Rückgabe im System`}
      action={
    ...
    >
      <div className="relative w-full max-w-sm">
        ...
        <Input
          placeholder="Ausleihe & Rückgabe suchen..."
          value={search}
        ...
        <Table className="[&_tbody_td]:px-6 [&_tbody_td]:py-2 [&_tbody_td]:text-base [&_tbody_td]:font-medium [&_tbody_tr:first-child_td]:pt-6 [&_tbody_tr:last-child_td]:pb-10">
          <TableHeader className="bg-secondary">
            <TableRow className="border-b border-input">
              ...
              <TableHead className="uppercase text-xs font-semibold text-secondary-foreground tracking-wider px-6 cursor-pointer select-none hover:text-foreground transition-colors" onClick={() => toggleSort('geplante_rueckgabe')}>
                <span className="inline-flex items-center gap-1">
                  Geplantes Rückgabedatum
                  {sortKey === 'geplante_rueckgabe' ? (sortDir === 'asc' ? <IconArrowUp size={14} /> : <IconArrowDown size={14} />) : <IconArrowsUpDown size={14} className="opacity-30" />}
                </span>
              </TableHead>
              <TableHead className="uppercase text-xs font-semibold text-secondary-foreground tracking-wider px-6 cursor-pointer select-none hover:text-foreground transition-colors" onClick={() => toggleSort('tatsaechliche_rueckgabe')}>
                <span className="inline-flex items-center gap-1">
                  Tatsächliches Rückgabedatum
                  {sortKey === 'tatsaechliche_rueckgabe' ? (sortDir === 'asc' ? <IconArrowUp size={14} /> : <IconArrowDown size={14} />) : <IconArrowsUpDown size={14} className="opacity-30" />}
                </span>
              </TableHead>
              ...
              <TableHead className="uppercase text-xs font-semibold text-secondary-foreground tracking-wider px-6 cursor-pointer select-none hover:text-foreground transition-colors" onClick={() => toggleSort('einsatzort')}>
                <span className="inline-flex items-center gap-1">
                  ...
                  {sortKey === 'einsatzort' ? (sortDir === 'asc' ? <IconArrowUp size={14} /> : <IconArrowDown size={14} />) : <IconArrowsUpDown size={14} className="opacity-30" />}
                </span>
              </TableHead>
              ...
              <TableHead className="uppercase text-xs font-semibold text-secondary-foreground tracking-wider px-6 cursor-pointer select-none hover:text-foreground transition-colors" onClick={() => toggleSort('bemerkung_ausleihe')}>
                <span className="inline-flex items-center gap-1">
                  Bemerkungen
                  {sortKey === 'bemerkung_ausleihe' ? (sortDir === 'asc' ? <IconArrowUp size={14} /> : <IconArrowDown size={14} />) : <IconArrowsUpDown size={14} className="opacity-30" />}
                </span>
              </TableHead>
          ...
          <TableBody>
            {sortRecords(filtered).map(record => (
              <TableRow key={record.record_id} className="hover:bg-muted/50 transition-colors cursor-pointer" onClick={(e) => { if ((e.target as HTMLElement).closest('button, [role="checkbox"]')) return; navigate(`/ausleihe-rueckgabe/${record.record_id}`); }}>
                <TableCell><span className="inline-flex items-center bg-secondary border border-[#bfdbfe] text-[#2563eb] rounded-[10px] px-2 py-1 text-sm font-medium">{getWerkzeugverwaltungDisplayName(record.fields.werkzeug)}</span></TableCell>
                <TableCell className="text-muted-foreground">{formatDate(record.fields.ausleihdatum)}</TableCell>
                <TableCell className="text-muted-foreground">{formatDate(record.fields.geplante_rueckgabe)}</TableCell>
                <TableCell className="text-muted-foreground">{formatDate(record.fields.tatsaechliche_rueckgabe)}</TableCell>
                ...
                <TableCell className="max-w-xs"><span className="truncate block">{record.fields.bemerkung_ausleihe ?? '—'}</span></TableCell>
                <TableCell>
            ...
            {filtered.length === 0 && (
              <TableRow>
                <TableCell colSpan={9} className="text-center py-16 text-muted-foreground">
                  {search ? 'Keine Ergebnisse gefunden.' : 'Noch keine Ausleihe & Rückgabe. Jetzt hinzufügen!'}
                </TableCell>
              </TableRow>
            )}
          </TableBody>
