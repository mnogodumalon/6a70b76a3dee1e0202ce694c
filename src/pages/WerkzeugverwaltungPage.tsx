...
export default function WerkzeugverwaltungPage() {
  const navigate = useNavigate();
  const [records, setRecords] = useState<Werkzeugverwaltung[]>([]);
  const [loading, setLoading] = useState(true);
  const [search, setSearch] = useState('');
  const [dialogOpen, setDialogOpen] = useState(false);
  const [editingRecord, setEditingRecord] = useState<Werkzeugverwaltung | null>(null);
  const [deleteTarget, setDeleteTarget] = useState<Werkzeugverwaltung | null>(null);
  ...
  async function loadData() {
    setLoading(true);
    try {
      setRecords(await LivingAppsService.getWerkzeugverwaltung());
    } finally {
      setLoading(false);
    }
  }

  async function handleCreate(fields: Werkzeugverwaltung['fields']) {
  ...
  async function handleUpdate(fields: Werkzeugverwaltung['fields']) {
    ...
    await LivingAppsService.updateWerkzeugverwaltungEntry(editingRecord.record_id, fields);
    await loadData();
  ...
  async function handleDelete() {
    ...
    await LivingAppsService.deleteWerkzeugverwaltungEntry(deleteTarget.record_id);
  ...
  return (
    <PageShell
      ...
      subtitle={`${records.length} Werkzeugverwaltung im System`}
      action={
    ...
    >
      <div className="relative w-full max-w-sm">
        ...
        <Input
          placeholder="Werkzeugverwaltung suchen..."
          value={search}
        ...
        <Table className="[&_tbody_td]:px-6 [&_tbody_td]:py-2 [&_tbody_td]:text-base [&_tbody_td]:font-medium [&_tbody_tr:first-child_td]:pt-6 [&_tbody_tr:last-child_td]:pb-10">
          <TableHeader className="bg-secondary">
            <TableRow className="border-b border-input">
              ...
              <TableHead className="uppercase text-xs font-semibold text-secondary-foreground tracking-wider px-6 cursor-pointer select-none hover:text-foreground transition-colors" onClick={() => toggleSort('werkzeugnummer')}>
                <span className="inline-flex items-center gap-1">
                  Werkzeug- / Inventarnummer
                  {sortKey === 'werkzeugnummer' ? (sortDir === 'asc' ? <IconArrowUp size={14} /> : <IconArrowDown size={14} />) : <IconArrowsUpDown size={14} className="opacity-30" />}
                </span>
              </TableHead>
              ...
              <TableHead className="uppercase text-xs font-semibold text-secondary-foreground tracking-wider px-6 cursor-pointer select-none hover:text-foreground transition-colors" onClick={() => toggleSort('lagerort')}>
                <span className="inline-flex items-center gap-1">
                  Standort / Lagerort
                  {sortKey === 'lagerort' ? (sortDir === 'asc' ? <IconArrowUp size={14} /> : <IconArrowDown size={14} />) : <IconArrowsUpDown size={14} className="opacity-30" />}
                </span>
              </TableHead>
              ...
              <TableHead className="uppercase text-xs font-semibold text-secondary-foreground tracking-wider px-6 cursor-pointer select-none hover:text-foreground transition-colors" onClick={() => toggleSort('pruefintervall_monate')}>
                <span className="inline-flex items-center gap-1">
                  Prüfintervall (Monate)
                  {sortKey === 'pruefintervall_monate' ? (sortDir === 'asc' ? <IconArrowUp size={14} /> : <IconArrowDown size={14} />) : <IconArrowsUpDown size={14} className="opacity-30" />}
                </span>
              </TableHead>
              ...
              <TableHead className="uppercase text-xs font-semibold text-secondary-foreground tracking-wider px-6 cursor-pointer select-none hover:text-foreground transition-colors" onClick={() => toggleSort('bemerkung_werkzeug')}>
                <span className="inline-flex items-center gap-1">
                  Bemerkungen
                  {sortKey === 'bemerkung_werkzeug' ? (sortDir === 'asc' ? <IconArrowUp size={14} /> : <IconArrowDown size={14} />) : <IconArrowsUpDown size={14} className="opacity-30" />}
                </span>
              </TableHead>
          ...
          <TableBody>
            {sortRecords(filtered).map(record => (
              <TableRow key={record.record_id} className="hover:bg-muted/50 transition-colors cursor-pointer" onClick={(e) => { if ((e.target as HTMLElement).closest('button, [role="checkbox"]')) return; navigate(`/werkzeugverwaltung/${record.record_id}`); }}>
                <TableCell className="font-medium">{record.fields.bezeichnung ?? '—'}</TableCell>
                <TableCell>{record.fields.werkzeugnummer ?? '—'}</TableCell>
                ...
                <TableCell>{record.fields.lagerort ?? '—'}</TableCell>
                ...
                <TableCell>{record.fields.pruefintervall_monate ?? '—'}</TableCell>
                ...
                <TableCell className="max-w-xs"><span className="truncate block">{record.fields.bemerkung_werkzeug ?? '—'}</span></TableCell>
                <TableCell>
            ...
            {filtered.length === 0 && (
              <TableRow>
                <TableCell colSpan={13} className="text-center py-16 text-muted-foreground">
                  {search ? 'Keine Ergebnisse gefunden.' : 'Noch keine Werkzeugverwaltung. Jetzt hinzufügen!'}
                </TableCell>
              </TableRow>
            )}
          </TableBody>
