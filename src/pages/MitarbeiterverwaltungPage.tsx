...
export default function MitarbeiterverwaltungPage() {
  ...
  async function loadData() {
    setLoading(true);
    try {
      setRecords(await LivingAppsService.getMitarbeiterverwaltung());
    } finally {
      setLoading(false);
    }
  }

  async function handleCreate(fields: Mitarbeiterverwaltung['fields']) {
  ...
  async function handleUpdate(fields: Mitarbeiterverwaltung['fields']) {
    ...
    await LivingAppsService.updateMitarbeiterverwaltungEntry(editingRecord.record_id, fields);
    await loadData();
  ...
  async function handleDelete() {
    ...
    await LivingAppsService.deleteMitarbeiterverwaltungEntry(deleteTarget.record_id);
        ...
        <Table className="[&_tbody_td]:px-6 [&_tbody_td]:py-2 [&_tbody_td]:text-base [&_tbody_td]:font-medium [&_tbody_tr:first-child_td]:pt-6 [&_tbody_tr:last-child_td]:pb-10">
          <TableHeader className="bg-secondary">
            <TableRow className="border-b border-input">
              ...
              <TableHead className="uppercase text-xs font-semibold text-secondary-foreground tracking-wider px-6 cursor-pointer select-none hover:text-foreground transition-colors" onClick={() => toggleSort('personalnummer')}>
                <span className="inline-flex items-center gap-1">
                  Personalnummer
                  {sortKey === 'personalnummer' ? (sortDir === 'asc' ? <IconArrowUp size={14} /> : <IconArrowDown size={14} />) : <IconArrowsUpDown size={14} className="opacity-30" />}
                </span>
              </TableHead>
              ...
              <TableHead className="uppercase text-xs font-semibold text-secondary-foreground tracking-wider px-6 cursor-pointer select-none hover:text-foreground transition-colors" onClick={() => toggleSort('handy')}>
                <span className="inline-flex items-center gap-1">
                  Handy-Nummer
                  {sortKey === 'handy' ? (sortDir === 'asc' ? <IconArrowUp size={14} /> : <IconArrowDown size={14} />) : <IconArrowsUpDown size={14} className="opacity-30" />}
                </span>
              </TableHead>
          ...
          <TableBody>
            {sortRecords(filtered).map(record => (
              <TableRow key={record.record_id} className="hover:bg-muted/50 transition-colors cursor-pointer" onClick={(e) => { if ((e.target as HTMLElement).closest('button, [role="checkbox"]')) return; navigate(`/mitarbeiterverwaltung/${record.record_id}`); }}>
                ...
                <TableCell><span className="inline-flex items-center bg-secondary border border-[#bfdbfe] text-[#2563eb] rounded-[10px] px-2 py-1 text-sm font-medium">{record.fields.abteilung?.label ?? '—'}</span></TableCell>
                <TableCell>{record.fields.telefon ?? '—'}</TableCell>
                <TableCell>{record.fields.handy ?? '—'}</TableCell>
