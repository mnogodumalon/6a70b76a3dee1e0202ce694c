/**
 * MitarbeiterverwaltungDialog — pre-generated create/edit dialog for Mitarbeiterverwaltung.
 *
 ...
 * MitarbeiterverwaltungDialogDefaults — NOT the entity field type: lookup fields accept
 * the bare KEY string (or LookupValue), applookup fields the bare record id
 * (or record URL); the dialog normalizes. Type prefill STATE with the export:
 *  ❌ useState<Partial<Mitarbeiterverwaltung['fields']>>({ … })   // LookupValue fields reject string prefills (TS2322)
 *  ✓ useState<MitarbeiterverwaltungDialogDefaults | undefined>(undefined)
 */
import { useState, useEffect, useMemo, useRef, useCallback } from 'react';
import type { Mitarbeiterverwaltung, LookupValue } from '@/types/app';
...
/** Widened prefill type for MitarbeiterverwaltungDialog.defaultValues — see file header. */
export type MitarbeiterverwaltungDialogDefaults = Omit<Mitarbeiterverwaltung['fields'], 'abteilung'> & {
    abteilung?: LookupValue | string;
  };
...
interface MitarbeiterverwaltungDialogProps {
  ...
  defaultValues?: MitarbeiterverwaltungDialogDefaults;
...
const NORMALIZE_LOOKUPS: Record<string, readonly { key: string; label: string }[]> = {
  abteilung: LOOKUP_OPTIONS['mitarbeiterverwaltung']?.['abteilung'] ?? [],
};
...
export function MitarbeiterverwaltungDialog({ open, onClose, onSubmit, defaultValues, recordId, enablePhotoScan = true, enablePhotoLocation = true }: MitarbeiterverwaltungDialogProps) {
  ...
  const REQUIRED_FIELDS = ['vorname', 'nachname', 'personalnummer'] as const;
  ...
  useEffect(() => {
    if (open) {
      setFields(applyDefaults(normalizedDefaults ?? {}, formEnhancements.defaults) as Partial<Mitarbeiterverwaltung['fields']>);
      setPreview(null);
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    try {
      ...
      const clean = cleanFieldsForApi(merged, 'mitarbeiterverwaltung');
      await onSubmit(clean as Mitarbeiterverwaltung['fields']);
      onClose();
    } catch (err) {
  ...
  async function handleAiExtract(file?: File) {
    ...
    try {
      ...
      const schema = `{\n  "vorname": string | null, // Vorname\n  "nachname": string | null, // Nachname\n  "geburtsdatum": string | null, // YYYY-MM-DD\n  "personalnummer": string | null, // Personalnummer\n  "abteilung": LookupValue | null, // Abteilung / Team (select one key: "installation" | "wartung" | "lager" | "montage" | "buero" | "sonstiges") mapping: installation=Installation, wartung=Wartung, lager=Lager, montage=Montage, buero=Büro, sonstiges=Sonstiges\n  "telefon": string | null, // Telefonnummer\n  "handy": string | null, // Handy-Nummer\n  "email": string | null, // E-Mail-Adresse\n}`;
  ...
  const DIALOG_INTENT = defaultValues ? 'Mitarbeiterverwaltung bearbeiten' : 'Mitarbeiterverwaltung hinzufügen';
  ...
  const fieldBlocks: Record<string, React.ReactNode> = {
    ...
    'personalnummer': (
      <div key="personalnummer" className="space-y-1.5">
        <Label htmlFor="personalnummer">Personalnummer <span className="text-destructive" aria-hidden="true">*</span></Label>
        <Input
          id="personalnummer"
          ...
          required
        />
        {showErrors && !fields.personalnummer && (
          <p className="text-xs text-destructive mt-1">Pflichtfeld</p>
        )}
      </div>
    ),
    'abteilung': (
      <div key="abteilung" className="space-y-1.5">
        <Label htmlFor="abteilung">Abteilung / Team</Label>
        <Select
          value={lookupKey(fields.abteilung) ?? ''}
          onValueChange={v => setFields(f => ({ ...f, abteilung: v === 'none' ? undefined : v as any }))}
        >
          <SelectTrigger id="abteilung" className="max-sm:h-11"><SelectValue placeholder="Welche Abteilung?" /></SelectTrigger>
          <SelectContent>
    ...
    'handy': (
      <div key="handy" className="space-y-1.5">
        <Label htmlFor="handy">Handy-Nummer</Label>
        <Input
          id="handy"
          value={fields.handy ?? ''}
          onChange={e => setFields(f => ({ ...f, handy: e.target.value }))}
        />
      </div>
    ),
    'email': (
  ...
  const FIELD_LABELS: Record<string, string> = {"vorname": "Vorname", "nachname": "Nachname", "geburtsdatum": "Geburtsdatum", "personalnummer": "Personalnummer", "abteilung": "Abteilung / Team", "telefon": "Telefonnummer", "handy": "Handy-Nummer", "email": "E-Mail-Adresse"};
